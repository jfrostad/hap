...
format_rasters <- function(these_rasters,
  ...
  keep_vars <- c('ADM0_CODE', 'ADM1_CODE', 'ADM2_CODE', 
                 ...
                 'pixel_id', 'year', 'pop', 'area_fraction', covs)
...
out_dir <- file.path(tmp_dir, 'output', 'bobby_covs')
...
regLoop <- function(region, build=T) {
  ...
  if (build) { #set TRUE if need to rebuild the table with new covs (longest part of process)
    ...
    #subset to only 2000/2017
    dt <- dt[year %in% c(2000, 2017)]
...
out[, diff := ghslurbanicity - data.table::shift(ghslurbanicity, n=1), by=.(ADM0_CODE, ADM1_CODE, ADM2_CODE)]

# finish up and save
tic('Saving ad2')
out_path <- file.path(out_dir, 'ad2_urbanicity.csv')
