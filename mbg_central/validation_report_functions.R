...
pull_country_draws_all_admins <- function(reg, periods, raked = "",
  ...
  make_country_list <- function(gaul) {
    ...
    ## Fix rasterize
    initial_raster <- rasterize(cropped_shapes, simple_raster, field = paste0('ADM', admin_level,'_CODE'))
    if(length(cropped_shapes[!cropped_shapes[[paste0('ADM', admin_level,'_CODE')]]%in%unique(initial_raster),])!=0) {
      rasterized_shape <- merge(rasterize(cropped_shapes[!cropped_shapes[[paste0('ADM', admin_level,'_CODE')]]%in%unique(initial_raster),], simple_raster, field = paste0('ADM', admin_level,'_CODE')), initial_raster)
    }
    if(length(cropped_shapes[!cropped_shapes[[paste0('ADM', admin_level,'_CODE')]]%in%unique(initial_raster),])==0) {
      rasterized_shape <- initial_raster
    }
