...
fractionally_rake_counts <- function(count_cell_pred,
  ...
  if(rake_subnational){
    subnational_country_code <- get_adm0_codes(countries_not_to_subnat_rake, shapefile_version = raking_shapefile_version)
...
calculate_fractional_rfs <- function(ndraws = ndraws,
  ...
  if (!is.null(countries_not_to_rake)) {
    fractional_rf[location_id %in% get_gbd_locs(reg = countries_not_to_rake, rake_subnational = F, shapefile_version = "current")$location_id, rf := 1]
  }
  if (!is.null(countries_not_to_subnat_rake)) {
    fractional_rf[location_id %in% get_gbd_locs(reg = countries_not_to_subnat_rake, rake_subnational = F, shapefile_version = "current")$location_id, rf := 1]
  }
