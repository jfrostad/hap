...
prep_cell_pred_for_frax_raking <- function(overs = overs,
                                           cell_pred = cell_pred,
                                           simple_raster = simple_raster,
                                           simple_polygon = simple_polygon,
                                           pixel_id = pixel_id,
  ...
  # rake fractional populations
  cell_pred <- cell_pred[, pop_raked := pop * pop_scalar] %>% 
    .[is.na(pop_raked), pop_raked := 0]
  
  ## Calc RFs ###################################################################
  message("raking rates")
  # Calculate Fractional Raking Factors
  fractional_rf <- calculate_fractional_rfs(overs    = overs,
                                            cell_pred = cell_pred,
                                            gbd       = gbd,
                                            output_dir  = output_dir,
                                            run_date  = run_date,
...
#' @export
fractionally_rake_rates <- function(fractional_rf = NULL, #if NULL, just reread it in from the saved csv
                                    cell_pred = cell_pred,
                                    simple_raster = simple_raster,
                                    simple_polygon = simple_polygon,
                                    pixel_id = pixel_id,
  ...
  output_dir <- ifelse(is.null(custom_output_folder), main_dir, custom_output_folder)
  ...
  #create the RFs and merge them to the cell_pred, then assign it to the function env (along w the link/rfs objs)
  prep_cell_pred_for_frax_raking(
      overs = overs,
      cell_pred = cell_pred,
      simple_raster = simple_raster,
      simple_polygon = simple_polygon,
  ...
  if (save_output) {
    ...
    paste0(output_dir, "/", indicator, "_unraked_", measure, "_c_admin_draws_eb_bin", 
  ...
  admin_0 <- admin_0[, (overs) := lapply(.SD, function(x) x / pop_raked), .SDcols=overs]
  admin_1 <- admin_1[, (overs) := lapply(.SD, function(x) x / pop_raked), .SDcols=overs]
  admin_2 <- admin_2[, (overs) := lapply(.SD, function(x) x / pop_raked), .SDcols=overs]
  
  ## save unraked rates aggregations
  if (save_output) {
    
    paste0(output_dir, "/", indicator, "_unraked_", measure, "_admin_draws_eb_bin", 
           age, "_", reg, "_", holdout, ".RData") %>%
      save(admin_0, admin_1, admin_2, sp_hierarchy_list, file = .)
  ...
  if(rake_method == "linear"){
    cell_pred <- cell_pred[, (overs) := lapply(.SD, function(x) x * rf), .SDcols=overs]
  }else{
  ...
  cell_pred[, (overs) := lapply(.SD, function(x) x * pop_raked), .SDcols=overs]
  ...
  if (save_output) {
    ...
    paste0(output_dir, "/", indicator, "_raked_", measure, "_c_admin_draws_eb_bin", age, "_", reg, "_", holdout, ".RData") %>%
      save(admin_0, admin_1, admin_2, sp_hierarchy_list, file = .)
  ...
  if (save_output) {
    ...
    paste0(output_dir, "/", indicator, "_raked_", measure, "_admin_draws_eb_bin", age, "_", reg, "_", holdout, ".RData") %>%
      save(admin_0, admin_1, admin_2, sp_hierarchy_list, file = .)
...
calculate_pop_scalars <- function(cell_pred = cell_pred,
  ...
  scalars %T>%
    write.csv(file = paste0(output_dir, "/", indicator, "_", stratum, "_pop_rf.csv"), row.names=F) %>% 
    return
}
...
#'
#' @export
calculate_fractional_rfs <- function(overs = overs,
                                     cell_pred = cell_pred,
                                     gbd = gbd,
                                     output_dir = output_dir,
                                     run_date = run_date,
  ...
  # save the raking factors, then return
  fractional_rf %T>%
    write.csv(., file = paste0(output_dir, "/", indicator, "_", stratum, "_rf.csv"), row.names = F) %>% 
    return
...
prep_shapes_for_raking <- function(reg, modeling_shapefile_version, raking_shapefile_version, field = "loc_id",
  ...
  # use SF package for speed improvement
  if (use_sf) {
    
    shapefile <- read_sf(shapefile_path) %>% 
      filter(ADM0_CODE %in% gaul_list) %>% 
      merge(location_metadata, by.x='ADM0_CODE', by.y='GAUL_CODE', all.x=T) %>% 
      tidyr::replace_na(list(loc_id=-1)) %>% 
      as_Spatial #TODO eventually rewrite all to SF, for now convert back
    
  } else { #default is the old SP method
