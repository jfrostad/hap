...
if (length(commandArgs()) >= 14) {
  ...
  measure <- commandArgs()[15]; message(measure)
} else {
...
# Re-assign holdout number (not sure why this needs done)
holdout         <- as.numeric(commandArgs()[11]); message(holdout)
...
if(raked){
  ...
  filename_rds <- paste0(share_dir, "/", indicator, "_cell_draws_raked_", measure, "_eb", pathaddin, ".rds")
  if (file.exists(filename_rds)) cell_pred<-readRDS(filename_rds)
  cell_pred <- cell_pred[[1]]
} else {
...
raster_list    <- build_simple_raster_pop(subset_shape, link_table = shapefile_version)
...
pop <- load_and_crop_covariates_annual(covs = 'worldpop_raked',
...
pop[, year := (min(year_list) - 1) + as.numeric(gsub("worldpop_raked.", "", variable))] # Converting "worldpop_raked.1" variables to actual years.
...
covdt[,pixel_id := pixel_id] #woo recycling
...
draw_colnames<-names(cell_pred) # names of draw columns
cell_pred<-cbind(cell_pred,pop) # Adding on the population and spatial information to cell_pred.
cell_pred<-cbind(cell_pred,covdt) # Adding stackers to cell_pred.
stopifnot(any(!(cell_pred[,pixel_id] != rep.int(covdt[,pixel_id], 16)))) # make sure everything combined properly
cols_to_aggregate <- c(covnames, draw_colnames) # indicate which columns to aggregate on
...
for(lvl in c(0,1,2)){
  ...
  if(length(missing_codes)==0){
    ...
    missing_locs<-merge(missing_locs,cell_pred[,c("pixel_id","pop","year",cols_to_aggregate),with=F],by="pixel_id",allow.cartesian=T)
...
region<-cell_pred[ADM0_CODE %in% region_adm0_list,lapply(.SD,weighted.mean,w=pop, na.rm=T),by=list(year), .SDcols=cols_to_aggregate]
...
adm0 <- adm0[ADM0_CODE %in% region_adm0_list,lapply(.SD,weighted.mean,w=pop, na.rm=T),by=list(ADM0_CODE,year), .SDcols=cols_to_aggregate]
if("1" %in% names(missing_admins)){adm1<-rbind(cell_pred,missing_admins[["1"]], fill=T)}else{adm1<-copy(cell_pred)}
adm1 <- adm1[ADM0_CODE %in% region_adm0_list,lapply(.SD,weighted.mean,w=pop, na.rm=T),by=list(ADM1_CODE,year), .SDcols=cols_to_aggregate]
if("2" %in% names(missing_admins)){adm2<-rbind(cell_pred,missing_admins[["2"]], fill=T)}else{adm2<-copy(cell_pred)}
adm2 <- adm2[ADM0_CODE %in% region_adm0_list,lapply(.SD,weighted.mean,w=pop, na.rm=T),by=list(ADM2_CODE,year), .SDcols=cols_to_aggregate]
...
save(admin_0,admin_1,admin_2,sp_hierarchy_list,
     file=paste0(share_dir, indicator, "_", ifelse(raked, paste0("raked_", measure), "unraked"), "_admin_draws_eb", pathaddin, ".RData"))
...
if(file.exists(paste0(share_dir, indicator, "_", ifelse(raked, paste0("raked_", measure), "unraked"), "_admin_draws_eb", pathaddin, ".RData"))){
  message("DONE!")
}else{
