...
library(LBDCore, lib.loc = "~/R/x86_64-pc-linux-gnu-library/3.6geo/")
library(colorout)
...
if(cores_to_use == "") cores_to_use <- 1
...
if(as.logical(skiptoinla) == FALSE){
  ...
  if(use_child_country_fes == TRUE | use_inla_country_fes == TRUE) {
    ...
    gaul_code <- rasterize_check_coverage(fe_subset_shape, fe_template, field = 'ADM0_CODE', link_table = modeling_shapefile_version)
...
if(!as.logical(skipinla)) {
  if(fit_with_tmb == FALSE) {
    ...
    model_fit <- fit_mbg(indicator_family = indicator_family,
                         ...
                         pardiso_license  = "~/sys/licenses/pardiso.lic",
                         verbose_output   = TRUE) 
  }
