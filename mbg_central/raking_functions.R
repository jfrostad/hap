...
rake_cell_pred <- function(cell_pred, 
  ...
  if(rake_subnational == F) {
    ...
    shapefile <- sp::merge(shapefile, location_metadata, by.x = "ADM0_CODE", by.y = "GAUL_CODE")
  } else {
  ...
  new_simple_polygon <- load_simple_polygon(gaul_list = NULL, ## doesn't matter since custom_shapefile specified
