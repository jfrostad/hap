...
makeInset <- function(i, loclist, 
  ...
  if (type=='scatter') {
    ...
    plot <- ggplot(dt, aes(x=1-hap_pct_mean, y=paf_mean, color=year %>% as.factor, group=ADM2_CODE)) + 
    ...
    if (type=='cloud_contour') {
      ...
      plot <- ggplot(dt, aes(x=1-hap_pct_mean, y=paf_mean)) + 
        ...
        geom_density_2d(data=dt[year==2000], color=colors[[1]], bins=contour_bw, size=.2) +
        geom_density_2d(data=dt[year==2018], color=colors[[2]], bins=contour_bw, size=.2)
...
makeRegFigure3s <- function(loc_list) {
  ...
  grid.text('2000', x = unit(.96, "npc"), y = unit(0.3, "npc"), gp = gpar(fontsize = 24, fontface = "bold"))
  grid.text('2018', x = unit(.96, "npc"), y = unit(0.5, "npc"), gp = gpar(fontsize = 24, fontface = "bold"))
  grid.text('2018', x = unit(.96, "npc"), y = unit(0.7, "npc"), gp = gpar(fontsize = 24, fontface = "bold"))
  dev.off()
