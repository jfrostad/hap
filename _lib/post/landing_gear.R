...
calcSummaryStats <- function(i, by_list, dt, labs_dt=adm_links, ind_cols, 
  ...
  if(collapsing) {
    ...
    agg[, paste0(mean_cols) := lapply(.SD,  Hmisc::wtd.mean, weights=pop), .SDcols=mean_cols, by=key(agg)] 
  ...
  #expand the dt to include all relevant terms for the trends
  trend_dt <- tidyr::crossing(agg[year %in% c(change_years$start, change_years$end)], 
                              expand.grid(term=c('change', 'change_rate'))) %>% 
    as.data.table %>% 
  ...
  #calculate trend vars
  trend_dt[term=='change', (ind_cols) := lapply(.SD, absChange), .SDcols=ind_cols, by=key(trend_dt)]
  trend_dt[term=='change_rate', (ind_cols) := lapply(.SD, relChange), .SDcols=ind_cols, by=key(trend_dt)]
  trend_dt[year==change_years$end, start_year := change_years$start] #capture the start year used to calculate trend
  
  #compile results
  agg <- list(
    agg[, term := 'lvl'], #note that these variables are capturing the level
    trend_dt[!is.na(start_year)] #keep final year
  ) %>% 
