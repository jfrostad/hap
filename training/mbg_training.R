...
core_repo <- '/share/code/geospatial/adesh/lbd_core'
...
if(as.logical(skiptoinla) == FALSE){
  ...
  if (nchar(fixed_effects) > 0) {
    ...
    effects <- trim(strsplit(fixed_effects, "\\+")[[1]])[1:3]
    measures <- trim(strsplit(fixed_effects_measures, "\\+")[[1]])[1:3]
  ...
  covchecklist <- check_for_uniform_covs()
  ...
  mesh_s_max_edge <- "c(2.5, 10)"
  mesh_s_offset <- "c(1, 2.5)"
...
time_fe=FALSE # TODO CONFIG THIS
